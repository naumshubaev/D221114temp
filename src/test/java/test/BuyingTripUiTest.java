...
public class BuyingTripUiTest {
    ...
    @DisplayName("Should display warnings with invalid values in the fields for Debit")
    ...
    @ParameterizedTest
    @CsvFileSource(resources = "/incorrectValues.cvs", numLinesToSkip = 1)
    @DisplayName("Should display warnings with invalid values in the fields for Credit")
    ...
    @Test
    @DisplayName("Should display error message with invalid values in the fields for Debit")
    void shouldShowWarningIfCardIsExpiredForPayment() {
        ...
        assertTrue(paymentPage.inputInvalidIsVisible(),"Should display error message with invalid values in the fields for Debit");
    }

    @Test
    @DisplayName("Should display error message with invalid values in the fields for Credit")
    void shouldShowWarningIfCardIsExpiredForCredit() {
        Card expiredCard = DataGenerator.getInvalidExpDateCard(-1);
        StartPage startPage = new StartPage();
        CreditPage creditPage = startPage.goToCreditPage();
        creditPage.fillData(expiredCard);
        assertTrue(creditPage.inputInvalidIsVisible(),"Should display error message with invalid values in the fields for Credit");
    }

    @Test
    @DisplayName("Should display error message with expired card for Debit")
    void shouldShowWarningIfExpirationDateMoreThan5YearsForPayment() {
        ...
        assertTrue(paymentPage.inputInvalidIsVisible(),"Should display error message with expired card for Debit");
    }

    @Test
    @DisplayName("Should display error message with expired card for Credit")
    void shouldShowWarningIfExpirationDateMoreThan5YearsForCredit() {
        Card invalidExpDateCard = DataGenerator.getInvalidExpDateCard(61);
        StartPage startPage = new StartPage();
        CreditPage creditPage = startPage.goToCreditPage();
        creditPage.fillData(invalidExpDateCard);
        assertTrue(creditPage.inputInvalidIsVisible(),"Should display error message with expired card for Credit");
    }
}
